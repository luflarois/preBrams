...
program pre
    ...
    do i=1,stepsBetDates
      write (*,fmt='("|",I2.2,"|",I4.4,"|",I2.2,"|",I2.2,"|",I2.2,"|")') i,atmosDate(i)%year &
      ,atmosDate(i)%month,atmosDate(i)%day,atmosDate(i)%hour
      write (*,fmt='("+--+----+--+--+--+")')
    enddo
    ...
    do iTime=1,stepsBetDates
      ...
      write(warningFileName,fmt='(I4.4,I2.2,I2.2,I2.2,".done")')  atmosDate(ITime)%year &
      ,atmosDate(Itime)%month,atmosDate(iTime)%day,atmosDate(iTime)%hour
      lunit=getUnit()
